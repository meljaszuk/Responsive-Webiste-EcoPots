...
window.addEventListener("hashchange", ()=>{
    if (window.location.hash === "#slide-menu") document.body.classList.add("body__lock");
...
function updateLargePot(item) {
    ...
    updateTotal();
    document.querySelector(".basket__volume-large-pot").textContent = volumeLargePot;
    document.querySelector(".basket__value-large-pot").textContent = `$ ` + valueLargePot.toFixed(2);
}
function updateSmallPot(item) {
    volumeSmallPot = volumeSmallPot + item;
    valueSmallPot = volumeSmallPot * PRICE_SMALL_POT;
    updateTotal();
    document.querySelector(".basket__volume-small-pot").textContent = volumeSmallPot;
    document.querySelector(".basket__value-small-pot").textContent = `$ ` + valueSmallPot.toFixed(2);
}
...
document.querySelector(".basket__plus.small-pot").addEventListener("click", function() {
    updateSmallPot(1);
});
document.querySelector(".basket__minus.small-pot").addEventListener("click", function() {
    if (volumeSmallPot > 0) updateSmallPot(-1);
});
document.querySelector(".basket__plus.large-pot").addEventListener("click", function() {
    updateLargePot(1);
});
document.querySelector(".basket__minus.large-pot").addEventListener("click", function() {
    if (volumeLargePot > 0) updateLargePot(-1);
});
